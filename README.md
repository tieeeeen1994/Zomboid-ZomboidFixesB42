...
- Admins and moderators get an admin hotbar, opened from a new sidebar button under Admin: a floating bar of one-click shortcuts to every admin tool (admin powers, admin panel windows, the Tools and Debug menus, weather and climate, meta events, player actions). Each shortcut keeps its own settings, such as a target player, a location, an item or a saved climate preset, toggles show whether they are on, and any image in the game can be its icon. Single player gets it with the -debug launch option (beta).
