...
- **Multiplayer fast forward.** Multiplayer has no speed controls: the game never puts them on screen for a client and ignores the speed keys. They are back, and each button is a vote. The game only speeds up once every living player has chosen fast forward, and then at the slowest speed anyone chose, using the same server multiplier as the debug `setTimeSpeed` command. If anyone goes back to normal speed, every vote is cleared and everyone has to choose again. As in single player, moving stops it, and so does a zombie close to anyone or anyone finishing an action; single player lets each player turn that last one off, here it always applies. Unlike single player, walking to something or driving stops it too, because a player moving at forty times the speed breaks the anti-cheat's speed limit and gets kicked.
